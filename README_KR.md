...
- **`no_std`이지만 `alloc`이 필요합니다**
  - 링크하려면 전역 할당자가 있어야 합니다.
  - dispatch도 힙을 씁니다. 가드 메모가 노드를 평가할 때마다 늘어나는 `Vec`이라, 노드를 하나라도 평가하는 dispatch는 할당합니다. 머신 dispatch에서 모든 가드가 거절됐을 때 debug 로그를 켜 두었다면 그 로그도 할당합니다.
  - `render`와 `verify`는 만드는 문서와 결과 목록 때문에 할당하고, `Machine::new`는 디버그 빌드에서 `verify::coverage`를 부릅니다.
  - 실행 중 힙 사용을 금지하는 타깃에는 아직 맞지 않습니다.
